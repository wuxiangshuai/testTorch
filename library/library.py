...
class NCFModel(torch.nn.Module):
    ...
    def forward(self, user, item):
        user_gmf_embedding = self.gmf_user_embedding(user.to(torch.int64))
        item_gmf_embedding = self.gmf_item_embedding(item.to(torch.int64))

        user_mlp_embedding = self.mlp_user_embedding(user.to(torch.int64))
        item_mlp_embedding = self.mlp_item_embedding(item.to(torch.int64))

        gmf_output = user_gmf_embedding * item_gmf_embedding

        # print(user_mlp_embedding.shape[0])
        # user_mlp_embedding = user_mlp_embedding.unsqueeze(1)
        print(user_mlp_embedding)
        print(item_mlp_embedding)
        if item_mlp_embedding.ndim == 3:
            item_mlp_embedding = item_mlp_embedding.squeeze()
            print(item_mlp_embedding)
...
for user in user_for_test:
    ...
    for batch in chunks(items_for_predict, 64):
        ...
        result = model(user, batch).view(-1).detach()
